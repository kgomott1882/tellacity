...
function planLabel(plan: PaidPlanKey): string {
  ...
  const zarNote =
    ...
    Number.isFinite(preview.fx_usd_zar) ? (
      <p className="mt-2 max-w-md text-xs leading-relaxed text-gray-600">
        ...
        (≈1 USD = {preview.fx_usd_zar.toFixed(2)} ZAR). Your bank may show another currency after
        checkout.
      </p>
  ...
  return (
    <div>
      <button
        ...
        className={className}
      >
        {busy ? "Redirecting…" : (children ?? `Upgrade to ${planLabel(plan)}`)}
      </button>
      {zarNote}
    </div>
  );
