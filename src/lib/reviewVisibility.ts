/** Supabase PostgREST `.or()` filter: public non-home reads include landing-hidden reviews. */
export const REVIEWS_PUBLIC_VISIBILITY_OR =
  "visibility.is.null,visibility.eq.visible,visibility.eq.landing_hidden" as const;
...
export const REVIEWS_PUBLIC_STATUS_AND_VISIBILITY_OR =
  "and(status.is.null,visibility.is.null),and(status.is.null,visibility.eq.visible),and(status.is.null,visibility.eq.landing_hidden),and(status.eq.published,visibility.is.null),and(status.eq.published,visibility.eq.visible),and(status.eq.published,visibility.eq.landing_hidden)" as const;
