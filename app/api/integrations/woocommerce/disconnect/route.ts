...
import { deleteWooCommerceIntegrationForBusiness } from "@/lib/woocommerceIntegrationServer";
...
export async function POST(request: Request) {
  ...
  const { ok, error } = await deleteWooCommerceIntegrationForBusiness(businessId);
  if (!ok) {
    console.error("[WooCommerce disconnect]", error);
    return NextResponse.json(
      { error: error ?? "Failed to disconnect WooCommerce." },
      { status: 500 },
    );
  }
