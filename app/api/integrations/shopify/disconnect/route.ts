...
import { deleteShopifyIntegrationForBusiness } from "@/lib/shopifyIntegrationServer";
...
export async function POST(request: Request) {
  ...
  const { ok, error } = await deleteShopifyIntegrationForBusiness(businessId);
  if (!ok) {
    console.error("[Shopify disconnect]", error);
    return NextResponse.json(
      { error: error ?? "Failed to disconnect Shopify." },
      { status: 500 },
    );
  }
