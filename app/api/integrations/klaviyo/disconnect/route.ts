...
import { deleteKlaviyoIntegrationForBusiness } from "@/lib/klaviyoIntegrationServer";
...
export async function POST(request: Request) {
  ...
  const { ok, error } = await deleteKlaviyoIntegrationForBusiness(businessId);
  if (!ok) {
    console.error("[Klaviyo disconnect]", error);
    return NextResponse.json(
      { error: error ?? "Failed to disconnect Klaviyo." },
      { status: 500 },
    );
  }
