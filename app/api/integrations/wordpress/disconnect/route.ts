...
import { deleteWordPressIntegrationForBusiness } from "@/lib/wordpressIntegrationServer";
...
export async function POST(request: Request) {
  ...
  const { ok, error } = await deleteWordPressIntegrationForBusiness(businessId);
  if (!ok) {
    console.error("[WordPress disconnect]", error);
    return NextResponse.json(
      { error: error ?? "Failed to disconnect WordPress." },
      { status: 500 },
    );
  }
