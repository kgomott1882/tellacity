...
import { deleteMagentoIntegrationForBusiness } from "@/lib/magentoIntegrationServer";
...
export async function POST(request: Request) {
  ...
  const { ok, error } = await deleteMagentoIntegrationForBusiness(businessId);
  if (!ok) {
    console.error("[Magento disconnect]", error);
    return NextResponse.json(
      { error: error ?? "Failed to disconnect Magento." },
      { status: 500 },
    );
  }
