...
import { deleteSalesforceIntegrationForBusiness } from "@/lib/salesforceIntegrationServer";
...
export async function POST(request: Request) {
  ...
  const { ok, error } = await deleteSalesforceIntegrationForBusiness(businessId);
  if (!ok) {
    console.error("[Salesforce disconnect]", error);
    return NextResponse.json(
      { error: error ?? "Failed to disconnect Salesforce." },
      { status: 500 },
    );
  }
