...
import { deleteSlackIntegrationForBusiness } from "@/lib/slackIntegrationServer";
...
export async function POST(request: Request) {
  ...
  const { ok, error } = await deleteSlackIntegrationForBusiness(businessId);
  if (!ok) {
    console.error("[Slack disconnect]", error);
    return NextResponse.json(
      { error: error ?? "Failed to disconnect Slack." },
      { status: 500 },
    );
  }
