...
import { deleteGoogleSheetsIntegrationForBusiness } from "@/lib/googleSheetsIntegrationServer";
...
export async function POST(request: Request) {
  ...
  const { ok, error } = await deleteGoogleSheetsIntegrationForBusiness(businessId);
  if (!ok) {
    console.error("[Google Sheets disconnect]", error);
    return NextResponse.json(
      { error: error ?? "Failed to disconnect Google Sheets." },
      { status: 500 },
    );
  }
  ...
  void logBusinessActivity({
    ...
    metadata: { provider: "google-sheets" },
  });
