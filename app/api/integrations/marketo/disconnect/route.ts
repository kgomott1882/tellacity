...
import { deleteMarketoIntegrationForBusiness } from "@/lib/marketoIntegrationServer";
...
export async function POST(request: Request) {
  ...
  const { ok, error } = await deleteMarketoIntegrationForBusiness(businessId);
  if (!ok) {
    console.error("[Marketo disconnect]", error);
    return NextResponse.json(
      { error: error ?? "Failed to disconnect Marketo." },
      { status: 500 },
    );
  }
