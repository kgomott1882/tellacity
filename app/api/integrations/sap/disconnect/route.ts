...
import { deleteSapIntegrationForBusiness } from "@/lib/sapIntegrationServer";
...
export async function POST(request: Request) {
  ...
  const { ok, error } = await deleteSapIntegrationForBusiness(businessId);
  if (!ok) {
    console.error("[SAP disconnect]", error);
    return NextResponse.json({ error: error ?? "Failed to disconnect SAP." }, { status: 500 });
  }
