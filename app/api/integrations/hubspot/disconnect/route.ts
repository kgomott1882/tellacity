...
import { deleteHubSpotIntegrationForBusiness } from "@/lib/hubspotIntegrationServer";
...
export async function POST(request: Request) {
  ...
  const { ok, error } = await deleteHubSpotIntegrationForBusiness(businessId);
  if (!ok) {
    console.error("[HubSpot disconnect]", error);
    return NextResponse.json(
      { error: error ?? "Failed to disconnect HubSpot." },
      { status: 500 },
    );
  }
