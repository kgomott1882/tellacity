...
import { deleteZendeskIntegrationForBusiness } from "@/lib/zendeskIntegrationServer";
...
export async function POST(request: Request) {
  ...
  const { ok, error } = await deleteZendeskIntegrationForBusiness(businessId);
  if (!ok) {
    console.error("[Zendesk disconnect]", error);
    return NextResponse.json(
      { error: error ?? "Failed to disconnect Zendesk." },
      { status: 500 },
    );
  }
