...
import { deleteNetsuiteIntegrationForBusiness } from "@/lib/netsuiteIntegrationServer";
...
export async function POST(request: Request) {
  ...
  const { ok, error } = await deleteNetsuiteIntegrationForBusiness(businessId);
  if (!ok) {
    console.error("[NetSuite disconnect]", error);
    return NextResponse.json(
      { error: error ?? "Failed to disconnect NetSuite." },
      { status: 500 },
    );
  }
