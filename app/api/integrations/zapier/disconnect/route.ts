...
import { deleteZapierIntegrationForBusiness } from "@/lib/zapierIntegrationServer";
...
export async function POST(request: Request) {
  ...
  const { ok, error } = await deleteZapierIntegrationForBusiness(businessId);
  if (!ok) {
    console.error("[Zapier disconnect]", error);
    return NextResponse.json(
      { error: error ?? "Failed to disconnect Zapier." },
      { status: 500 },
    );
  }
