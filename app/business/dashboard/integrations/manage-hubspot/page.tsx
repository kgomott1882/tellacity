...
export default function ManageHubSpotPage() {
  ...
  const [status, setStatus] = useState<HubSpotStatus>({ loading: true });
  ...
  const handleDisconnect = async () => {
    ...
    const confirmed = window.confirm(
      "Disconnect HubSpot from Tellacity? CRM sync and review visibility in HubSpot will stop until you connect again.",
    );
    if (!confirmed) return;
    ...
    setError("");
    try {
      await dashboardApiPost<{ ok: boolean }>("/api/integrations/hubspot/disconnect", {
        business_id: businessId.trim(),
      });
      router.push("/business/dashboard/integrations?disconnected=hubspot");
      router.refresh();
    } catch (err) {
  ...
  if (status.loading) {
    return (
      <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        <p className="text-sm text-gray-500">Loading HubSpot connection…</p>
      </div>
    );
  }
  ...
  if (!status.connected) {
    return (
      <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        <h1 className="text-lg font-semibold text-[#0E0E0E]">HubSpot</h1>
        <p className="mt-2 text-sm text-gray-600">
          This business is not connected to HubSpot yet.
        </p>
        <div className="mt-6 flex flex-wrap gap-3">
          <Link
            href={hubSpotConnectPath(businessId)}
    ...
    <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-lg font-semibold text-[#0E0E0E]">HubSpot</h1>
          <p className="mt-1 text-sm text-gray-600">
            Tellacity can reach your HubSpot portal using the private app token you saved.
          </p>
        </div>
      ...
      <dl className="mt-6 space-y-4 rounded-xl border border-gray-100 bg-gray-50/80 p-4 text-sm">
        {status.portal_id ? (
          <div>
            ...
            <dd className="mt-1 font-medium text-[#0E0E0E]">{status.portal_id}</dd>
          </div>
        ) : null}
        {status.account_type ? (
          <div>
            ...
            <dd className="mt-1 text-gray-700 capitalize">
              {status.account_type.toLowerCase().replace(/_/g, " ")}
            </dd>
          </div>
        ) : null}
        <div>
      ...
      <section className="mt-6">
        ...
        <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-gray-600">
          ...
          <li>
            When HubSpot sync is enabled for your account, contacts and deals can show Tellacity
            reviews and feedback automatically.
          </li>
          <li>
            Until then, export or reference review data from your{" "}
            <Link href="/business/dashboard/manage-reviews" className="text-[#1FAF9E] hover:underline">
              Reviews
            </Link>{" "}
            dashboard and link records in HubSpot manually.
          </li>
        </ul>
      </section>
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        ...
        <Link
          href={integrationConnectorPath("hubspot")}
