...
export default function ManageMagentoPage() {
  ...
  const [status, setStatus] = useState<MagentoStatus>({ loading: true });
  ...
  const loadStatus = useCallback(async () => {
    ...
    try {
      ...
      setStatus({
        ...
        store_code: json.store_code ?? "default",
  ...
  const handleDisconnect = async () => {
    ...
    const confirmed = window.confirm(
      "Disconnect Magento from Tellacity? Order-based review invites will stop until you connect again.",
    );
    if (!confirmed) return;
    ...
    setError("");
    try {
      await dashboardApiPost<{ ok: boolean }>("/api/integrations/magento/disconnect", {
        business_id: businessId.trim(),
      });
      router.push("/business/dashboard/integrations?disconnected=magento");
      router.refresh();
    } catch (err) {
  ...
  if (status.loading) {
    return (
      <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        <p className="text-sm text-gray-500">Loading Magento connection…</p>
      </div>
    );
  }
  ...
  if (!status.connected) {
    return (
      <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        <h1 className="text-lg font-semibold text-[#0E0E0E]">Magento</h1>
        <p className="mt-2 text-sm text-gray-600">
          This business is not connected to Magento yet.
        </p>
        <div className="mt-6 flex flex-wrap gap-3">
          <Link
            href={magentoConnectPath(businessId)}
    ...
    <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-lg font-semibold text-[#0E0E0E]">Magento</h1>
          <p className="mt-1 text-sm text-gray-600">
            Tellacity can reach your store using the integration access token you saved.
          </p>
        </div>
      ...
      <dl className="mt-6 space-y-4 rounded-xl border border-gray-100 bg-gray-50/80 p-4 text-sm">
        <div>
          <dt className="text-xs font-medium text-gray-500">Store URL</dt>
          <dd className="mt-1 font-medium text-[#0E0E0E] break-all">{status.site_url}</dd>
        </div>
        <div>
          <dt className="text-xs font-medium text-gray-500">Store code</dt>
          <dd className="mt-1 text-gray-700">{status.store_code}</dd>
        </div>
        <div>
      ...
      <section className="mt-6">
        ...
        <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-gray-600">
          ...
          <li>
            When order-based review invites are enabled for your account, completed Magento orders
            can trigger invitations automatically.
          </li>
          <li>
            Until then, use{" "}
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        ...
        <Link
          href={integrationConnectorPath("magento")}
