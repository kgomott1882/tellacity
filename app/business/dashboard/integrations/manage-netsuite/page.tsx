...
function environmentLabel(accountId: string): string {
  return /_SB[0-9]+$/i.test(accountId) ? "Sandbox" : "Production";
}

export default function ManageNetsuitePage() {
  ...
  const [status, setStatus] = useState<NetsuiteStatus>({ loading: true });
  ...
  const handleDisconnect = async () => {
    ...
    const confirmed = window.confirm(
      "Disconnect NetSuite from Tellacity? Customer feedback sync will stop until you connect again.",
    );
    if (!confirmed) return;
    ...
    setError("");
    try {
      await dashboardApiPost<{ ok: boolean }>("/api/integrations/netsuite/disconnect", {
        business_id: businessId.trim(),
      });
      router.push("/business/dashboard/integrations?disconnected=netsuite");
      router.refresh();
    } catch (err) {
  ...
  if (status.loading) {
    return (
      <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        <p className="text-sm text-gray-500">Loading NetSuite connection…</p>
      </div>
    );
  }
  ...
  if (!status.connected) {
    return (
      <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        <h1 className="text-lg font-semibold text-[#0E0E0E]">NetSuite</h1>
        <p className="mt-2 text-sm text-gray-600">
          This business is not connected to NetSuite yet.
        </p>
        <div className="mt-6 flex flex-wrap gap-3">
          <Link
            href={netsuiteConnectPath(businessId)}
    ...
    <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-lg font-semibold text-[#0E0E0E]">NetSuite</h1>
          <p className="mt-1 text-sm text-gray-600">
            Tellacity can reach your NetSuite account using the TBA credentials you saved.
          </p>
        </div>
      ...
      <dl className="mt-6 space-y-4 rounded-xl border border-gray-100 bg-gray-50/80 p-4 text-sm">
        ...
        <div>
          ...
          <dd className="mt-1 font-mono text-xs text-gray-700">{status.account_id}</dd>
        </div>
        <div>
          <dt className="text-xs font-medium text-gray-500">Environment</dt>
          <dd className="mt-1 text-gray-700">{environmentLabel(status.account_id)}</dd>
        </div>
        <div>
      ...
      <section className="mt-6">
        ...
        <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-gray-600">
          <li>
            Your TBA credentials are stored server-side and only used by Tellacity integrations.
          </li>
          <li>
            When NetSuite sync is enabled for your account, customer records can show Tellacity
            reviews and NPS alongside ERP data.
          </li>
          <li>
            ...
            dashboard and link records in NetSuite manually.
          </li>
        </ul>
      </section>
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        ...
        <Link
          href={integrationConnectorPath("netsuite")}
