...
export default function ManageWooCommercePage() {
  ...
  const [status, setStatus] = useState<WooStatus>({ loading: true });
  ...
  const handleDisconnect = async () => {
    ...
    const confirmed = window.confirm(
      "Disconnect WooCommerce from Tellacity? Review invites from new orders will stop until you connect again.",
    );
    if (!confirmed) return;
    ...
    setError("");
    try {
      await dashboardApiPost<{ ok: boolean }>("/api/integrations/woocommerce/disconnect", {
        business_id: businessId.trim(),
      });
      router.push("/business/dashboard/integrations?disconnected=woocommerce");
      router.refresh();
    } catch (err) {
  ...
  if (status.loading) {
    return (
      <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        <p className="text-sm text-gray-500">Loading WooCommerce connection…</p>
      </div>
    );
  }
  ...
  if (!status.connected) {
    return (
      <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        <h1 className="text-lg font-semibold text-[#0E0E0E]">WooCommerce</h1>
        <p className="mt-2 text-sm text-gray-600">
          This business is not connected to WooCommerce yet.
        </p>
        <div className="mt-6 flex flex-wrap gap-3">
          <Link
            href={wooCommerceConnectPath(businessId)}
    ...
    <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-lg font-semibold text-[#0E0E0E]">WooCommerce</h1>
          <p className="mt-1 text-sm text-gray-600">
            Tellacity can reach your store using the REST API keys you saved.
          </p>
        </div>
      ...
      <section className="mt-6">
        ...
        <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-gray-600">
          <li>Your API keys are stored server-side and are only used by Tellacity integrations.</li>
          <li>
            When order-based review invites are enabled for your account, completed WooCommerce
            orders can trigger invitations automatically.
          </li>
          <li>
            Until then, use{" "}
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        ...
        <Link
          href={integrationConnectorPath("woocommerce")}
