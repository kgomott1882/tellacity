...
export default function ManageMarketoPage() {
  ...
  const [status, setStatus] = useState<MarketoStatus>({ loading: true });
  ...
  const loadStatus = useCallback(async () => {
    ...
    try {
      ...
      setStatus({
        loading: false,
        connected: true,
        rest_endpoint: json.rest_endpoint ?? "",
        munchkin_id: json.munchkin_id ?? null,
  ...
  const handleDisconnect = async () => {
    ...
    const confirmed = window.confirm(
      "Disconnect Marketo from Tellacity? Program and smart campaign automations will stop until you connect again.",
    );
    if (!confirmed) return;
    ...
    setError("");
    try {
      await dashboardApiPost<{ ok: boolean }>("/api/integrations/marketo/disconnect", {
        business_id: businessId.trim(),
      });
      router.push("/business/dashboard/integrations?disconnected=marketo");
      router.refresh();
    } catch (err) {
  ...
  if (status.loading) {
    return (
      <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        <p className="text-sm text-gray-500">Loading Marketo connection…</p>
      </div>
    );
  }
  ...
  if (!status.connected) {
    return (
      <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        <h1 className="text-lg font-semibold text-[#0E0E0E]">Marketo</h1>
        <p className="mt-2 text-sm text-gray-600">
          This business is not connected to Marketo yet.
        </p>
        <div className="mt-6 flex flex-wrap gap-3">
          <Link
            href={marketoConnectPath(businessId)}
    ...
    <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-lg font-semibold text-[#0E0E0E]">Marketo</h1>
          <p className="mt-1 text-sm text-gray-600">
            Tellacity can reach your Marketo instance using the REST credentials you saved.
          </p>
        </div>
      ...
      <dl className="mt-6 space-y-4 rounded-xl border border-gray-100 bg-gray-50/80 p-4 text-sm">
        <div>
          <dt className="text-xs font-medium text-gray-500">REST API endpoint</dt>
          <dd className="mt-1 font-medium text-[#0E0E0E] break-all">{status.rest_endpoint}</dd>
        </div>
        {status.munchkin_id ? (
          <div>
            <dt className="text-xs font-medium text-gray-500">Munchkin Account ID</dt>
            <dd className="mt-1 text-gray-700">{status.munchkin_id}</dd>
          </div>
        ) : null}
        <div>
      ...
      <section className="mt-6">
        ...
        <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-gray-600">
          <li>Your Client Secret is stored server-side and is only used by Tellacity integrations.</li>
          <li>
            When Marketo program automations are enabled for your account, smart campaigns can
            include Tellacity review links and triggers automatically.
          </li>
          <li>
            ...
            into Marketo emails or program tokens.
          </li>
        </ul>
      </section>
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        ...
        <Link
          href={integrationConnectorPath("marketo")}
