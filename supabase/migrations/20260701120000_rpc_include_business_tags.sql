-- Expose businesses.tags on profile slug RPC and category listing RPC so public UIs can show keywords.
...
comment on function public.get_business_by_slug(text) is
  'Public business profile row by slug; includes tags for keyword chips.';
...
comment on function public.get_top_businesses_for_category_global(text, text, double precision, int, int) is
  'Category directory listing; includes tags for keyword chips next to category.';
