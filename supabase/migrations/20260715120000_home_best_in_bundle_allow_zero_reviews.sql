-- Homepage Best-in: take top p_final_limit by trust/review metrics even when live_review_count is 0
-- (removes the previous "published reviews only" gate on the inner pick).
...
comment on function public.get_home_best_in_bundle(text, text[], int, int, double precision) is
  'Homepage Best-in: top p_final_limit businesses per slug by live trust/review metrics (0 reviews allowed).';
