-- Admin overview: two-way split only — consumer_users = all auth users not in the business bucket
-- (workspace team or custom-domain auth email). Drops other_users from the RPC return shape.
...
comment on function public.admin_get_overview_stats() is
  'Admin dashboard aggregates. business_users: workspace team in auth OR custom-domain auth email; consumer_users: all other auth users. total_users = count(auth.users).';
