-- Recent activity: use person_name (avoid PL/pgSQL "name" conflicts). Business column = company name
-- (business_profiles.business_name first, then businesses.name). Person column prefers auth metadata
-- signup_first_name / signup_last_name (set at signup) then profiles.
...
COMMENT ON FUNCTION public.admin_get_recent_activity(integer) IS
  'Admin overview: person_name + business subtitle (company name). Requires is_admin.';
