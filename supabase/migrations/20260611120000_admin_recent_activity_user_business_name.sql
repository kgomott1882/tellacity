-- Recent activity: show signup "Business" column from business_profiles + auth user_metadata.
-- Replaces 20260505120000 drift: that version left user subtitle empty and joined profiles on user_id.
...
COMMENT ON FUNCTION public.admin_get_recent_activity(integer) IS
  'Admin overview: reviews, businesses, signups with business name on signup (business_profiles + signup_company_name). Requires is_admin.';
