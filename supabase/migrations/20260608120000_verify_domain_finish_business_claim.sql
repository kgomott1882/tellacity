-- Domain OTP completion: activate draft or claim listing + business_owners in one transaction.
-- SECURITY DEFINER avoids "permission denied for table users" when hosted DB has triggers/FKs
-- that reference public.users or other objects the PostgREST role cannot touch reliably.

create or replace function public.verify_domain_finish_business_claim(
  p_business_id uuid,
  p_code text
)
returns jsonb
...
begin
  ...
  v_email := lower(nullif(trim(both from coalesce(auth.jwt() ->> 'email', '')), ''));
  ...
  -- Match app logic: sessionEmailDomainMatchesBusinessWebsite (normalizeWebsiteDomain rules).
  ...
  select v.id, v.code, v.expires_at
  into v_vid, v_stored_code, v_expires
...
comment on function public.verify_domain_finish_business_claim(uuid, text) is
  'After domain OTP: activate pending_verification draft or claim active listing; upsert business_owners; consume OTP. Call with user JWT.';

revoke all on function public.verify_domain_finish_business_claim(uuid, text) from public;
grant execute on function public.verify_domain_finish_business_claim(uuid, text) to authenticated;
