-- Replace legacy verify_review_token (consumer_otps / magic_token) with draft OTP + review_drafts publish.
drop function if exists public.verify_review_token(uuid);
...
comment on function public.verify_review_token(uuid, text) is
  'Guest OTP verify: p_token = review_drafts.id, p_code = 6-digit code from review_otps; publishes review and deletes draft.';

grant execute on function public.verify_review_token(uuid, text) to anon;
grant execute on function public.verify_review_token(uuid, text) to authenticated;
grant execute on function public.verify_review_token(uuid, text) to service_role;
