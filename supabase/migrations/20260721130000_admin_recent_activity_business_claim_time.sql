-- Recent activity: sort business rows by claim time when a listing is claimed long after import.
-- Previously used only businesses.created_at, so claims on old listings never appeared near the top.
...
BEGIN
  ...
  WITH items AS (
    SELECT
      ...
      NULLIF(TRIM(BOTH FROM COALESCE(b.country_code::text, '')), '')::text AS country_code
    ...
    SELECT
      CASE
        WHEN act.activity_at > b.created_at + interval '1 minute'
        THEN 'business_claim'::text
        ELSE 'business'::text
      END AS item_type,
      b.id::uuid,
      CASE
        WHEN act.activity_at > b.created_at + interval '1 minute'
        THEN 'Business claimed'::text
        ELSE 'Business created'::text
      END AS title,
      COALESCE(
      ...
      act.activity_at::timestamptz AS created_at,
      NULLIF(TRIM(BOTH FROM COALESCE(b.country_code::text, '')), '')::text AS country_code
    ...
    LEFT JOIN LATERAL (
      SELECT
        GREATEST(
          b.created_at,
          COALESCE(bo.created_at::timestamptz, '-infinity'::timestamptz),
          COALESCE(
            (
              SELECT MAX(bd.consumed_at)::timestamptz
...
COMMENT ON FUNCTION public.admin_get_recent_activity(integer, integer) IS
  'Admin overview: recent activity (newest first). Business rows use max(listing created_at, business_owners.created_at, domain OTP consumed_at) so claims surface; item_type business_claim vs business. Requires is_admin.';
