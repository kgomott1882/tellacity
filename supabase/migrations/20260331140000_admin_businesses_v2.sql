-- Admin businesses list v2: paginated search + filters + total count.

alter table public.businesses add column if not exists submission_status text;
...
comment on function public.admin_list_businesses_v2(text, text, text, integer, integer) is
  'Admin: paginated businesses with optional search (name / id) and status / submission filters.';

grant execute on function public.admin_list_businesses_v2(text, text, text, integer, integer)
  to authenticated;
...
comment on function public.admin_count_businesses_v2(text, text, text) is
  'Admin: total count for admin_list_businesses_v2 with same filters.';

grant execute on function public.admin_count_businesses_v2(text, text, text) to authenticated;
