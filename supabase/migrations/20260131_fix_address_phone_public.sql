-- One-time fix: make address, phone, email from dashboard show on the public profile.
-- Run this once in Supabase Dashboard → SQL Editor → paste and Run.

-- 1) Ensure businesses has the columns the dashboard saves (address, city, country_code, email, phone, etc.)
...
-- 2) Function the public page calls: return one business by slug including address, phone, email
-- Drop first when the return type has changed; PostgreSQL cannot change it with create or replace.
