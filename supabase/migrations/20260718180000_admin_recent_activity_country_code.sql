-- Admin recent activity: expose business country_code for review (and business-created) rows.
...
COMMENT ON FUNCTION public.admin_get_recent_activity(integer, integer) IS
  'Admin overview: recent activity with limit + offset (newest first). country_code from businesses for review/business rows. Requires is_admin.';
